...
class SimpleBluetoothIO: NSObject {
    ...
    func writeValue(value: Int8) {
        ...
        print("write value " + value.description + " to " + characteristic.uuid.uuidString)
...
extension SimpleBluetoothIO: CBCentralManagerDelegate {
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        print("something " + peripheral.debugDescription)
        peripheral.discoverServices(nil)
    }

    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        print("try to connect " + peripheral.debugDescription)
    ...
    func centralManagerDidUpdateState(_ central: CBCentralManager) {
        ...
        if central.state == .poweredOn {
            print("try to scan at " + serviceUUID)
